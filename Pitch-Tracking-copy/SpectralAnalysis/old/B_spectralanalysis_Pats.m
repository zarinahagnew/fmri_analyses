...
for each_subject = 1:npatients
    ...
    for iblock=1:8
        nfreqs=length(gooddata_spec.goodpitchdata(1).data(1,:))/2+1;        
        for itrial=1:10
            %gooddata_spec.goodpitchdata(iblock).data
            if sum(isnan(gooddata_spec.goodpitchdata(iblock).data(itrial,:))) = 0 ;
            spectrum(iblock).data=my_spectrum(gooddata_spec.goodpitchdata(iblock).wholetrial,fs,nfreqs)
        end
    end
